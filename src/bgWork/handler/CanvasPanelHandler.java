...
public class CanvasPanelHandler extends PanelHandler {
	...
	public void ActionPerformed(MouseEvent e) {
		...
		case 0:
			System.out.println("呼叫selectByClick");
			selectByClick(e);
			break;
		case 1:
		case 2:
	...
	void selectByClick(MouseEvent e) {
		...
		System.out.println("還原所有port select");
		setSelectAllLine();
		for (int i = 0; i < members.size(); i++) {
			if (isInside(members.elementAt(i), e.getPoint()) == true && isSelect == false) {
				...
				case 0:
					...
					((BasicClass) members.elementAt(i)).setSelectedPort(new AreaDefine()
							.getArea(members.elementAt(i).getLocation(), members.elementAt(i).getSize(), e.getPoint()));
					isSelect = true;
					findSelectLine((MyShape) members.elementAt(i));
					break;
				case 1:
					((UseCase) members.elementAt(i)).setSelect(true);
					selectComp.add(members.elementAt(i));
					((UseCase) members.elementAt(i)).setSelectedPort(new AreaDefine()
							.getArea(members.elementAt(i).getLocation(), members.elementAt(i).getSize(), e.getPoint()));
					isSelect = true;
					findSelectLine((MyShape) members.elementAt(i));
					break;
	...
	void findSelectLine(MyShape selectShape) {
		for (MyLine line : lines) {
			boolean isSelect;
			if (((line).getFromShape() == selectShape && (line).getFromSide() == selectShape.getSelectedPort())
					|| ((line).getToShape() == selectShape && (line).getToSide() == selectShape.getSelectedPort())) {
				System.out.println("點到port");
				line.setSelect(true);
			}
//			switch (core.isLine(line)) {
//			case 0:
//				isSelect = ((AssociationLine) line).isSelect();
//
//				if ((((AssociationLine) line).getFromShape() == selectShape
//						&& ((AssociationLine) line).getFromSide() == selectShape.getSelectedPort())
//						|| (((AssociationLine) line).getToShape() == selectShape
//								&& ((AssociationLine) line).getToSide() == selectShape.getSelectedPort())) {
//					System.out.println("點到port");
//					((AssociationLine) line).setSelect(true);
//				}
//				break;
//			case 1:
//				isSelect = ((CompositionLine) line).isSelect();
//				System.out.println(isSelect);
//				break;
//			case 2:
//				isSelect = ((GeneralizationLine) line).isSelect();
//				System.out.println(isSelect);
//				break;
//			case 3:
//				isSelect = ((DependencyLine) line).isSelect();
//				System.out.println(isSelect);
//			default:
//				break;
//			}
	...
	void setSelectAllLine() {
		for (JPanel line : lines) {
			setSelectAllType(line, false);
		}
	}
